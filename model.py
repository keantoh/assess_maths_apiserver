...
class Child(Base):
    ...
    favColour = Column('fav_colour', Integer, ForeignKey(
        'Colour.colour_id'), nullable=False)
    favAnimal = Column('fav_animal', Integer, ForeignKey(
        'Animal.animal_id'), nullable=False)
    colourName = relationship(
        'Colour', primaryjoin='Child.favColour == Colour.colourId', uselist=False)
    animalName = relationship(
        'Animal', primaryjoin='Child.favAnimal == Animal.animalId', uselist=False)
...
class Result(Base):
    ...
    resultId = Column('result_id', String, primary_key=True,
                      default=lambda: str(uuid4()))
    childId = Column('child_id', Integer, ForeignKey(
        'Child.child_id'), nullable=True)
    sessionStartTime = Column('session_start_time', DateTime, nullable=False)
    questionId = Column('question_id', Integer, ForeignKey(
